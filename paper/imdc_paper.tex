...
\begin{abstract}
\noindent
Dengue is expanding across Brazil; 2024 produced a record epidemic roughly four times any prior
year. Accurate, calibrated probabilistic forecasts support preparedness, but which of statistical,
machine-learning, deep-learning, and mechanistic approaches forecasts best, especially under an
outbreak year's distribution shift, is unsettled. We report a reproducible, leakage-controlled
comparison of five probabilistic forecasting model families, 24 models and variants in total, for the 3rd Infodengue--Mosqlimate
Dengue Challenge (IMDC 2026), which requires weekly, state-level, full-interval forecasts for the
26 mandatory Brazilian states. All models are evaluated through a single backtesting harness over
four official retrospective seasons, one of which contains the 2024 outbreak, and scored by the
weighted interval score (WIS). We report four results with implications beyond this challenge.
First, no model is best across seasons: global recurrent networks are the most accurate single
models in normal seasons, yet the one we deployed is the worst of all on the 2024 outlier, while an
ensemble never incurs any member's worst-season failure. Second, the best model depends on the evaluation metric: magnitude-weighted mean WIS and
scale-free normalized WIS reward accuracy on different subsets of a highly skewed set of states.
Third, a conformal recalibration, a post-hoc widening of the ensemble's intervals tuned to fix
their overconfidence, improves accuracy and calibration at negligible cost (mean WIS, a
magnitude-weighted score, 1234 to 1162; normalized WIS, its scale-free counterpart, 0.595 to
0.561; out-of-sample reduction of 6.2 percent), acting as asymmetric insurance: little cost in an
ordinary season, but it prevents the catastrophic under-prediction an extreme one would otherwise
cause. Fourth, added model complexity did not pay: observed-climate covariates and hierarchical
spatial pooling both failed to improve forecasts, and an internal-holdout hyperparameter gain
reversed on the true backtest. The best model is disease-specific: for chikungunya, gradient
boosting alone outperforms the ensemble. All code, the backtesting harness, per-state scored
predictions, and validated submissions for four official tracks are released, with an automated
test suite covering leakage, scoring, and every model.
\end{abstract}

\section*{Author summary}
\noindent
Brazil carries one of the world's largest dengue burdens, and 2024 brought an epidemic without
precedent in the national record. Public-health agencies increasingly rely on forecasts that give
a full range of plausible outcomes with calibrated uncertainty, not just one predicted number.
Which method to trust is an open question, especially in an exceptional season, when accurate
forecasts matter most. We compared five model families, from simple seasonal baselines to
gradient boosting, deep learning, and a mechanistic epidemic model, using identical retrospective
tests across four dengue seasons in all 26 mandatory Brazilian states. No method is best
everywhere: deep learning was the most accurate approach in ordinary seasons but failed badly during
the 2024 outbreak, while a simple ensemble that averages the models was the most dependable. The
apparent winner also changes depending on how performance is scored, a caution for forecasting
leaderboards. Finally, a light statistical recalibration that stretched the ensemble's prediction
intervals just enough to fix their overconfidence improved both accuracy and reliability. Adding
more data or model complexity did not help. At the challenge's own validation-round results
...
Prior work on dengue forecasting is extensive but leaves that question open in three specific
ways. First, most published comparisons rank models on point accuracy. A systematic review and
network meta-analysis of 59 dengue-forecasting studies from 2014 to 2024 ranked methods on
root-mean-square error and found simple statistical models competitive with machine learning at
lower data and implementation cost \citep{benjarattanaporn2026}. Point accuracy does not measure
whether a prediction interval is calibrated, which is what a preparedness decision needs. Second,
the probabilistic comparisons that do exist mostly target short horizons. The largest, an
evaluation across more than 180 locations worldwide, forecasts one to three months ahead
and finds ensembles consistently among the top performers \citep{wu2025}; a Brazilian statistical
model forecasts probabilistic epidemic bands, the activity range a coming season is expected to
fall within, rather than a weekly trajectory \citep{freitas2025}. A full-season target, here 16 to
67 weeks ahead because a 15-week reporting gap separates the training cutoff from the first target
week, is a different problem: no recent case data is available near the target, so the models that
win at one month need not win here. Third, the inaugural sprint reported that no single model
excelled across locations and years, and observed that its three scoring rules did not agree on a
winner \citep{araujo2026}. The question we take up is a different one: not whether separate scoring
rules disagree, which is expected of rules measuring different properties, but whether two
aggregations of the \emph{same} proper scoring rule, one weighted by case magnitude and one
scale-free, select different models. That disagreement cannot be attributed to the scores measuring
different things, and it determines what a leaderboard reports.
...
retrospective seasons (2022--23 through 2025--26) for the 26 mandatory states. The design trades
breadth for depth relative to multi-country evaluations \citep{wu2025}: one country, two diseases,
a full-season horizon, and every model sharing one leakage-audited pipeline whose tests, provenance
manifest, and per-state scored predictions are public. That depth is what makes the negative
results and the metric-disagreement finding below checkable rather than anecdotal. We make the
...
model in Table~\ref{tab:byfold}, and the 2025--26 season (fold 4) is only partially resolved in the
available data and is reported separately.
...
rather than the Poisson head more common in count models) is the strongest single model in the two
ordinary seasons, leading fold 1 outright at WIS 348 and placing second on fold 3 at 324, behind
only its own pinball-trained variant at 321 (Section~\ref{sec:extended}); the three ensembles beat
it on fold 1, which is the point of combining. It is nonetheless the worst of all models on the 2024
outlier, fold 2 (WIS 4131), worse even than a naive baseline, because the surge lies outside its
training distribution and its intervals, derived from its fitted distribution instead of built
...
overall, Section~\ref{sec:methods}) is the strongest of the three deployed families on the outlier
season (WIS 3238), and unlike LightGBM, which showed a sharper trade-off (strongest of the two on
the outlier at WIS 3332 yet the weakest of the serious models on the ordinary season fold 3, WIS
665), XGBoost remains reasonably competitive instead of collapsing in the ordinary seasons (WIS 376
and 484 in folds 1 and 3). Two models outside the deployed ensemble handle the outlier season better
still: the surge template at WIS 3118 and the semi-mechanistic trajectory model at 3371, both for
the same reason, which is that each builds its intervals by resampling whole historical seasons and
so widens them precisely when the target departs from the recent past
...
Ensemble (inverse-WIS)    & \textbf{313} & 3625 & 387 & 401 \\
GRU (negative-binomial)   & 348 & 4131 & 324 & 542 \\
GRU (pinball-trained)     & 361 & 3736 & \textbf{321} & 455 \\
XGBoost                   & 376 & 3238 & 484 & 468 \\
Climatological            & 386 & 3588 & 516 & 309 \\
Mechanistic               & 468 & 3371 & 551 & 314 \\
Surge template            & 489 & \textbf{3118} & 511 & 329 \\
...
The deep network is the best single model in ordinary seasons and the worst of all on the 2024
outlier; the ensemble avoids every model's worst-season failure. The conformal-recalibrated
ensemble (Section~\ref{sec:conformal}) is shown alongside its members. For legibility the figure
plots the eight models of the deployed comparison; Table~\ref{tab:byfold} adds the two extended-set
models of Section~\ref{sec:extended}.}
\label{fig:byfold}
\end{figure}
...
positive value of the same size would mean 1.5 times worse. One property of this metric constrains
how its values should be read: because it is a fully-crossed pairwise comparison, each model's
score depends on the set of models it is compared against, so the figures below apply to the six
models in Fig~\ref{fig:stability} and shift if the comparison set changes. They are not directly
comparable to a relative WIS computed elsewhere against a different set. Within this set, the deep
network has the best mean
skill ($-0.42$) but
...
corner: modest mean skill ($-0.09$) but the lowest variability of any model scored here ($0.04$;
the seasonal-naive baseline is excluded from this comparison, since relative WIS is measured
against it and its variability is zero by construction), because its
...
it needs to (the 80 percent interval above needed only a negligible correction). This step lifts the
ensemble to the top of the table rather than leaving it a hedge against any one member's worst
case: the recalibrated ensemble has the lowest mean WIS of any model we evaluated
(1162, against 1234 for the plain ensemble and 1190 for the best of the three deployed families,
XGBoost; normalized WIS 0.561 versus 0.595) and is better
calibrated at the tails (empirical coverage 46/74/87/94 versus 43/72/82/86), as shown in
Fig~\ref{fig:conformal}. That lead is narrow at the top: the surge template
(Section~\ref{sec:extended}) reaches 1168, and the 6-unit gap between them is not statistically
significant (Section~\ref{sec:significance}). What separates them is where each earns its score,
and we return to this in Section~\ref{sec:extended}. Out of sample, on the three seasons not used for calibration, the reduction
...
50/80/90/95 is better. Best value in each column is in bold. The surge template and the
pinball-trained GRU come from the extended set of Section~\ref{sec:extended}; S5 Table lists all
24 models and variants evaluated, including those not retained.}
...
Surge template            & 1168 & 0.564 & 0.447 & 64 & 88 & 93 & 96 \\
...
GRU (pinball-trained)     & 1274 & 0.615 & 0.366 & 33 & 56 & 70 & 78 \\
LightGBM                  & 1301 & 0.628 & 0.549 & 48 & 80 & 89 & 93 \\
Seasonal-naive            & 1379 & 0.666 & 0.468 & 43 & 78 & 87 & 92 \\
GRU (negative-binomial)   & 1394 & 0.673 & 0.385 & 27 & 50 & 60 & 67 \\
...
\caption{\textbf{Effect of conformal recalibration on the dengue ensemble.} (A) Change in mean WIS
per season, conformal-recalibrated minus Vincentization ensemble, so negative means recalibration
helps. The asymmetry is the point: a gain of 277 WIS units in the 2024 outlier season against
costs of 5 and 9 units in the two ordinary seasons that follow it, roughly thirty to fifty times
smaller. (B) Interval calibration: empirical versus nominal coverage. Recalibration moves the
outer intervals (90, 95) toward the diagonal.}
\label{fig:conformal}
\end{figure}
...
$[56.9, 87.8]$) and on XGBoost by 27.9 $([8.8, 47.3])$; on ordinary seasons
...
The same test also bounds what we can claim at the top of the table. Against the surge template,
the best-scoring single model on mean WIS (Section~\ref{sec:extended}), the conformal ensemble's
advantage over all four seasons is 5.9 WIS with a 95 percent interval of $[-26.2, 37.4]$, which
includes zero: on raw mean WIS the two are statistically indistinguishable, because the outbreak
season the surge template handles well dominates that average. Restricted to the three ordinary
seasons, the ensemble beats it by 73.9 $([54.5, 91.7])$, an interval well clear of zero. We
therefore do not claim the ensemble is significantly the most accurate model in raw aggregate. We
claim something narrower and better supported: it is the most accurate in the seasons that recur,
it is never the worst in any season, and it is the only model near the top of the table that also
passes the tuning-fold check used to select it.
...
which fell all the way to the weakest non-baseline model in Table~\ref{tab:leaderboard} under this
metric (0.549, still its value there since LightGBM itself was not adopted): XGBoost's more
...
skill is dominated by a few states. Espírito Santo (gray) is excluded from the challenge.}
\label{fig:skillmap}
\end{figure}
...
horizon-dependent: it is the strongest of the four models stratified here at short lead times (normalized WIS 0.255 at 16 to 27 weeks, bettered only by its pinball-trained variant at 0.227, Section~\ref{sec:extended})
...
\subsection{Nine further models, and what their failures identify}\label{sec:extended}
The five families above are the ones the deployed pipeline uses. We built and scored nine more,
partly on our own initiative and partly by reimplementing techniques described by other teams in
this challenge and its 2024 predecessor. S5 Table reports all of them on the identical backtest;
four warrant discussion here because their outcomes locate where the predictable signal in this
problem is, and is not. Section~\ref{sec:why} draws the common thread through them.

The \textbf{surge template} is the strongest single model we built by mean WIS (1168, against 1190
for XGBoost). It carries no covariates at all. It circularly shifts each historical season to align
its peak, averages those into one normalized template shape, and forecasts by drawing a peak-week
offset and a log-normal gain against that template. Its advantage is entirely a property of the
outbreak season, where it scores 3118, the best in Table~\ref{tab:byfold}, because
resampling whole historical seasons produces intervals that widen when the target is unlike the
recent past. The two first-pass classical models make the same point far more starkly. Prophet
records the best outbreak-season score of any model we built, 2812, and SARIMAX the second best,
2979, yet they finish last and 16th of 24 overall, because the very wide intervals that cover the
2024 surge are ruinous in the ordinary seasons (fold 3 WIS 2636 and 1464 respectively).
Outbreak-season skill alone is not evidence of a good forecaster, and the model that best covers
the worst season in the record can still be the worst model in the study. The same width is dead weight in an ordinary season, where it over-covers badly
(64 percent empirical coverage at the nominal 50 percent level) and pays for that in dispersion:
its normalized WIS excluding 2024 is 0.447, eighth of the twelve models in
Table~\ref{tab:leaderboard}, and its
tuning-fold normalized WIS is 0.444 against the deployed ensemble's 0.306. A model can therefore
lead the raw aggregate while failing the criterion used to select models, which is the
metric-disagreement result of Section~\ref{sec:metric} arriving through an independent route.

A \textbf{pinball-trained GRU}, identical in architecture to the deployed deep network but fitted
to the quantile loss directly instead of through a negative-binomial likelihood, is significantly
the better of the two as a standalone model (mean WIS 1274 versus 1394, paired difference 120.0,
95 percent CI $[91.0, 150.5]$), has the best ordinary-season normalized WIS of any single model
(0.366), and is still overconfident, though less so (33/56/70/78 against 27/50/60/67). It is
nonetheless the worse ensemble member: substituted for the negative-binomial variant it loses on
every ensemble-level metric. Being a better model and being a better ensemble member are different
properties, and the second depends on error correlation with the other members rather than on
accuracy alone.

The \textbf{classical statistical family} deserves explicit mention because a Prophet model and a
SARIMAX model from another team were named among the best-performing dengue state-level entries at
this edition's validation round \citep{imdc2026webinar}. Ours were not. A first
Prophet pass scored 2177 and a first SARIMAX pass 1419, both worse than seasonal-naive. Rebuilding
each against the specific gaps we could identify, flat growth, real climate and ocean-index
regressors, and a seasonally stratified residual bootstrap for Prophet, improved it to 1520, a 30
percent reduction that still left it well short. For SARIMAX, the order selected on one fold did
not transfer to another regime (1752 when frozen, 1528 with a per-fold search), which is the same
tuning-transfer failure documented in Section~\ref{sec:negative}. We report these as negative
results rather than omitting them: at a 16 to 67 week horizon with no recent case data near the
target, the autoregressive structure both families rely on has little left to condition on.

Finally, \textbf{conditioning the surge template on climate} made it worse, not better (1406 versus
1168, degrading on every metric). Centring the forecast season's gain distribution on a regression
against the pre-cutoff temperature anomaly added noise without signal, because an eight-week
anomaly reading is a weak proxy for the climate of a season more than a year ahead. This is the
same conclusion as the covariate ablation in Section~\ref{sec:negative}, reached from the opposite
direction: the climate information that helps here is the forward-looking seasonal forecast and the
persistent ocean indices, not a recent local observation.
...
outputs. Among the models in Table~\ref{tab:operational}, the gradient-boosted model (XGBoost) has the lowest onset-timing error (3.7 weeks mean
absolute
...
The same holds for dengue specifically: across more than 180 locations worldwide, ensembles
ranked consistently among the top performers at one-to-three-month horizons, including where
individual models faltered \citep{wu2025}. Our result extends that to a full-season horizon and
identifies the mechanism in this setting: the ensemble is never worst because its members fail in
different seasons, and conformal recalibration then converts that robustness into the best overall
score. The challenge exists to give Brazil's Ministry of Health a pre-season view of what the
coming year may bring \citep{araujo2026}, and that use carries a concrete recommendation from these
results: prefer a recalibrated ensemble to the single best-scoring model on a leaderboard. The
season in which a leaderboard-topping model fails is, by construction, the season with no
precedent in the training record, which is exactly the season a preparedness decision depends on
most.
...
\citep{makridakis2020,benjarattanaporn2026}. The pattern recurs one sprint earlier: in IMDC24, a
Bayesian baseline model using no climate covariates at all was among the most consistently
competitive submissions across states and seasons \citep{araujo2026}, echoing our own finding that
a covariate-free climatological baseline and a historical-trajectory bootstrap are hard to beat
with more heavily engineered alternatives. It recurs again against an architecturally close
competitor: on monthly dengue counts in Freetown, Sierra Leone, a parsimonious autoregressive count
model (INGARCH with a negative-binomial observation model) beat a bidirectional LSTM with a
negative-binomial head, the closest published analogue of our own GRU, on distributional accuracy,
with the deep model preferred only where tail reliability mattered most \citep{sesay2026}. Three
independent settings, two continents, and two surveillance systems give the same answer, which
makes it a property of the forecasting regime rather than of any one pipeline. The same predecessor sprint offers an independent caution about tuning ensembles on
...
\subsection{What separated the models that worked from the ones that did not}\label{sec:why}
Across the 24 models and variants we evaluated, spanning statistical, machine-learning, deep-learning, mechanistic, and
template-based approaches, performance tracked one property more closely than model class: how each
model generates its prediction intervals, and therefore how those intervals behave when the target
season is unlike the training record. WIS decomposes exactly into a dispersion penalty, paid for
interval width whether or not it is needed, and directional penalties, paid when the observation
falls outside the interval (Section~\ref{sec:methods-scoring}). Which of those two costs a model
incurs is close to a design decision.

Models that build intervals by resampling whole historical seasons, the semi-mechanistic bootstrap
and the surge template, produce width that scales with historical variability. They pay dispersion
continuously and are rarely caught out: on the 2024 outlier the surge template ranks third and the
mechanistic model eighth of the twenty models for which we have per-season scores (3118 and 3371),
while neither is competitive in ordinary seasons, where that width is simply a cost. Models that
derive intervals from a fitted parametric distribution, principally the two deep networks, pay
almost no dispersion and are sharp when the season is ordinary, which is why the two of them hold
the best ordinary-season normalized WIS of any single model, 0.366 for the pinball-trained variant
and 0.385 for the deployed one. The same sharpness is catastrophic when the
observation lands far outside: the negative-binomial GRU covers 27 percent of observations at its
nominal 50 percent level and records the worst outlier-season score of any model, worse than a naive
baseline. This is a trade-off, not a ranking, and it explains why a single leaderboard position is a
poor summary of a forecasting model.

Three further patterns are worth stating because each was learned from a failure. First, covariates
helped only when they were forward-looking. Observed local climate, whether added to the boosted
model (Section~\ref{sec:negative}) or used to condition the surge template's gain
(Section~\ref{sec:extended}), degraded forecasts in both cases; the seasonal climate forecast and
the persistent ocean indices, which carry information about the target window rather than the
recent past, are retained in every deployed model. At a 16 to 67 week horizon this is close to a
definitional point: a covariate observed before the cutoff has decayed to noise by the target week
unless the quantity itself persists. Second, autoregressive structure has little to condition on at
this horizon, which is the common thread linking the Prophet and SARIMAX results to the general
weakness of recent-lag features. These families are strong at the one-to-three-month horizons where
comparable studies deploy them \citep{wu2025,sesay2026} and weak here, so the appropriate conclusion
is about horizon, not about the families. Third, every intervention that improved a model in
isolation had to be re-checked in combination, and two of them reversed: the pinball-trained GRU is
significantly better standalone yet a worse ensemble member, and the shallower boosted configuration
won the standalone backtest yet lost at the ensemble level. Ensemble value is a function of error
correlation, which standalone accuracy does not measure.
...
simulating reporting delay. Two leakage disciplines are enforced centrally in the harness, not in
individual models. All covariate access is filtered to dates at or before the training cutoff, and
lag and rolling features are computed from the continuous cutoff-filtered history rather than from the
gapped target window. Both are checked by automated regression tests, so a leak fails the test suite
...
Model comparisons are tested by a block bootstrap over whole state-season units
(Section~\ref{sec:significance}) rather than by a Diebold-Mariano test \citep{diebold1995}, the
more common choice for comparing two forecast series. The Diebold-Mariano statistic is built for a
single long series of forecast errors with weak serial dependence. Our panel is 26 short state
series that are cross-sectionally correlated, with one season an order of magnitude harder than
the rest, so resampling whole state-season blocks preserves the dependence structure that an
asymptotic test on pooled errors would assume away.
...
\bibitem{wu2025} Wu S, Meyer AG, Clemente L, Stolerman LM, Lu F, Majumder A, Verbeeck R, Masyn S, Santillana M. Ensemble approaches for short-term dengue fever forecasts: a global evaluation study. \emph{Proc Natl Acad Sci USA} 122:e2422335122 (2025).
\bibitem{freitas2025} Freitas LP, da Cruz Ferreira DA, Lana RM, C\^amara DCP, Portella TP, Carvalho MS, et al. A statistical model for forecasting probabilistic epidemic bands for dengue cases in Brazil. \emph{Infect Dis Model} 10(4):1479--1487 (2025).
\bibitem{sesay2026} Sesay MM, Ngunyi A, Imboga H. Probabilistic forecasting of monthly dengue cases using epidemiological and climate signals: a BiLSTM-negative binomial model versus mechanistic and count-model baselines. \emph{PLOS Glob Public Health} 6(3):e0005404 (2026).
\bibitem{benjarattanaporn2026} Benjarattanaporn P, Adewo D, Sutton A, Lee A, Dodd PJ. Dengue forecasting models: a systematic review and network meta-analysis of relative performance. \emph{medRxiv} 2026.02.18.26346534 (2026).
\bibitem{diebold1995} Diebold FX, Mariano RS. Comparing predictive accuracy. \emph{J Bus Econ Stat} 13(3):253--263 (1995).
