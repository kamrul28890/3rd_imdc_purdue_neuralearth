...
was run\footnote{This test and the hyperparameter-tuning test below were performed on the
LightGBM-based pipeline before it was superseded by XGBoost (Section~\ref{sec:methods}) and before
a later data refresh; both figures are the original findings and have not been re-verified on the
current XGBoost-based pipeline or the current data. We report them because the underlying
methodological lessons -- observed local weather is not the load-bearing climate signal, and
internal-holdout hyperparameter gains do not transfer to the true backtest -- do not depend on
which gradient-boosted implementation they were discovered with, and re-deriving both from scratch
was out of scope for this revision.}). A feature-importance analysis showed why: the nine
...
backtest as the main comparison. $^{\dagger}$The climate-covariate and hyperparameter-tuning tests
were run on the LightGBM configuration before it was superseded by XGBoost and before a later data
refresh (see footnote in text); their methodological lessons still apply but the exact figures are
not re-verified on the current pipeline.}
\label{tab:ablation}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Intervention} & \textbf{Effect} & \textbf{Reason} \\
\midrule
Observed-climate covariates$^{\dagger}$ (LightGBM) & WIS 1299 $\rightarrow$ 1342 & 5.3\% importance; signal is climate \\
...
Hyperparameter tuning$^{\dagger}$ & internal holdout $+5\%$, & internal holdout misrepresents \\
