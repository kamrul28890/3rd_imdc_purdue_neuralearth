...
an ensemble-level result can disagree even when both are individually rigorous. Together these
...
conclusions can be implementation-specific rather than a fixed property of the problem.

\begin{table}[t]
\centering\small
\caption{\textbf{Interventions that did not help, plus one that reversed on re-verification but
still did not survive ensemble-level scrutiny (negative results throughout).} Each was evaluated on
...
either -- none of these four rows changed the deployed model.}
\label{tab:ablation}
\begin{tabular}{@{}lll@{}}
\toprule
