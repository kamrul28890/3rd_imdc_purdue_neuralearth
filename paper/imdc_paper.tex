...
excelled across locations and years, but did not isolate why rankings move, nor test whether the
choice of aggregate metric itself changes the winner \citep{araujo2026}.
...
recent past. The first-pass SARIMAX makes the same point more starkly: it records the best
outbreak-season score of any model we built, 2979, while finishing 16th of 24 overall, because the wide
intervals that serve it in 2024 are ruinous in the three ordinary seasons (fold 3 WIS 1464).
Outbreak-season skill alone is not evidence of a good forecaster. The same width is dead weight in an ordinary season, where it over-covers badly
(64 percent empirical coverage at the nominal 50 percent level) and pays for that in dispersion:
its normalized WIS excluding 2024 is 0.447, among the worst in Table~\ref{tab:leaderboard}, and its
...
The \textbf{classical statistical family} deserves explicit mention because the predecessor sprint's
Prophet and SARIMAX entries were among its strongest \citep{araujo2026}. Ours were not. A first
...
not transfer to another regime (1752 when frozen, 1529 with a per-fold search), which is the same
...
continuously and are rarely caught out: both are among the best on the 2024 outlier (3371 and 3118)
and neither is competitive in ordinary seasons, where that width is simply a cost. Models that
derive intervals from a fitted parametric distribution, principally the two deep networks, pay
almost no dispersion and are sharp when the season is ordinary, which is why the deep network has
the best ordinary-season skill of any single model. The same sharpness is catastrophic when the
