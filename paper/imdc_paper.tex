...
(mean WIS, a magnitude-weighted score, 1281 to 1216; normalized WIS, its scale-free counterpart,
0.585 to 0.555; out-of-sample reduction of 5.5 percent). It acts as asymmetric insurance: the
...
ordinary seasons, folds 1 and 3 (WIS 339 and 354), but is the worst of all models on the 2024
outlier, fold 2 (WIS 3955), worse even than a naive baseline, because the surge lies outside its
training distribution and its intervals, derived from its fitted distribution rather than built
empirically, are too narrow to cover it. The gradient-boosted model (LightGBM) shows the opposite profile: it is
...
each other. A log relative WIS of $-0.54$, for instance, means a model whose WIS is on average
$e^{0.54}\approx1.7$ times smaller than the baseline's, i.e. about 1.7 times more accurate; a
positive value of the same size would mean 1.7 times worse. The deep network has the best mean
skill of any model on this scale ($-0.54$) but
also the highest variability of any model except naive ($0.20$), the quantitative signature of its
ordinary-season strength and outlier-season failure. The mechanistic model sits at the opposite
corner: modest mean skill ($-0.11$) but the lowest variability of any model ($0.03$), because its
...
markedly better consistency (std $0.14$ and $0.11$ for the median and conformal ensembles), which is
...
level, of the climatological, gradient-boosted,
...
conformalized quantile regression (CQR) used inside LightGBM (Section~\ref{sec:methods-conformal}
...
median of 100 becomes approximately [$-4$, 204], clipped to [0, 204] for non-negativity, after the
1.73 factor: the median is never touched, only the interval stretches, and it stretches only where
it needs to (the 80 percent interval above needed no correction at all). The recalibrated ensemble is the most accurate model overall
...
normalized WIS of
$[0.404, 0.635]$, and the intervals of the leading models overlap (Fig~\ref{fig:significance}A), a
...
The conformal recalibration improves on the median ensemble by a mean 64.8 WIS (95 percent CI
$[50.1, 79.5]$) and on the best single model, LightGBM, by 83.5 $([53.4, 115.8])$; on ordinary seasons
the deep network beats LightGBM by 153.3 $([97.8, 215.8])$; and the conformal ensemble beats the naive
baseline by 448 $([387, 514])$. All four intervals exclude zero. The lesson is methodological as much
...
with 95 percent intervals; all three comparisons lie left of zero, so the conformal ensemble
significantly beats the median ensemble and LightGBM, and the deep network significantly beats
LightGBM on ordinary seasons.}
...
interval only actually contains the truth 32 percent of the time, and its nominal 95 percent
interval only 75 percent of the time, and it is under-covered in every season
...
horizon-dependent: it is the best model at short lead times (normalized WIS 0.25 at 16 to 27 weeks)
but degrades at the longest horizons (0.38 at 52 to 67 weeks), where it converges with the
gradient-boosted model. The conformal ensemble is the most stable model across horizons and is never
...
is dominated by under-prediction (a mean of 2703 WIS units, against 619 for dispersion and 3 for
...
out. First, the gradient-boosted model alone is the best forecaster (WIS 82.3), ahead of the ensemble
(91.6) and every other model (Table~\ref{tab:chik}); it is best in every season
...
than the others (mean WIS 125 versus a range of 68 to 143 across seasons), in contrast to the
...
\textbf{Model} & \textbf{Fold 1} & \textbf{Fold 2} & \textbf{Fold 3} & \textbf{Fold 4} \\
\midrule
LightGBM                  & \textbf{81.5} & \textbf{125.0} & \textbf{67.8} & \textbf{20.6} \\
Ensemble (Vincentization) & 88.3 & 142.0 & 73.8 & 25.2 \\
Climatological            & 94.3 & 143.3 & 75.3 & 27.6 \\
Seasonal-naive            & 92.8 & 152.3 & 77.1 & 32.5 \\
Mechanistic               & 90.7 & 166.3 & 77.8 & 29.2 \\
Naive                     & 181.3 & 145.7 & 75.3 & 48.6 \\
\bottomrule
\end{tabular}
\end{table}
...
(Table~\ref{tab:city}), against 0.56 and 0.64 at the state level. Interval coverage is also poorer (50
percent coverage of 36 percent for dengue cities), because many mid-sized municipalities have episodic,
...
and normalized WIS averaged over the target cities and four seasons.}
\label{tab:city}
\begin{tabular}{@{}llrrc@{}}
\toprule
\textbf{Disease} & \textbf{Model} & \textbf{WIS} & \textbf{nWIS} & \textbf{Cov.\ 50 (\%)} \\
\midrule
Dengue (15 cities)      & Climatological & \textbf{128.7} & \textbf{0.60} & 36 \\
                        & Seasonal-naive & 138.1 & 0.65 & 31 \\
                        & Naive          & 141.2 & 0.66 & 30 \\
\addlinespace
Chikungunya (10 cities) & Climatological & \textbf{32.2} & \textbf{0.87} & 54 \\
...
outputs. The gradient-boosted model has the lowest onset-timing error (3.1 weeks mean absolute
error, against 3.3 for the conformal ensemble), the climatological model has the lowest peak-timing
error (4.4 weeks, against 4.6 for the ensemble), and the gradient-boosted model again gives the
least biased total-season case count (a log-ratio of $-0.08$, i.e. predicting about 92 percent of
the true total, an 8 percent under-prediction, at the median unit, against 20
percent for the ensemble and 55 percent for the mechanistic model). Every model under-predicts case
counts at the true peak week specifically, by 46 to 76 percent at the median unit, more than it
...
\midrule
Ensemble (conformal)   & 3.3 & 4.6 & $-0.73$ & $-0.22$ \\
LightGBM               & \textbf{3.1} & 4.7 & \textbf{$-0.54$} & \textbf{$-0.08$} \\
Climatological         & 3.6 & \textbf{4.4} & $-0.95$ & $-0.35$ \\
GRU (deep learning)    & 3.9 & 5.2 & $-0.62$ & $-0.17$ \\
Mechanistic            & 3.7 & 5.0 & $-1.43$ & $-0.77$ \\
Seasonal-naive         & 3.7 & 4.6 & $-0.86$ & $-0.34$ \\
Naive                  & 4.5 & 16.2 & $-1.01$ & $0.00^{*}$ \\
\bottomrule
\end{tabular}

\smallskip
\footnotesize $^{*}$Naive's total-cases figure is not genuine skill: carrying the last observed
value forward for 52 weeks over- and under-predicts in roughly equal measure across the season, so
early- and late-season errors cancel in the sum even though naive is the worst model by every other
measure in this table, including a 16-week peak-timing error. It is a reminder that a single
season-total number, like a single aggregate WIS, can look better than a model's real skill.
\end{table}
...
independently per fold: baselines and the climatological quantile model, LightGBM with conformalized
quantile regression, the GRU deep ensemble, and the semi-mechanistic trajectory bootstrap. The
climatological, LightGBM, and GRU outputs (not the mechanistic model, which is evaluated separately
...
family's output directly (LightGBM for chikungunya-state, climatological for both city tracks; see
...
independently, three of them (climatological, LightGBM, GRU) are combined by per-quantile median
...
too few to fit a tree ensemble, so LightGBM \citep{ke2017} is instead fit on a
...
ever happens at that fold's own four-times-a-year official target window. Predictive intervals are
