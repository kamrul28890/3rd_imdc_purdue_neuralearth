...
each other. The deep network has the best mean skill of any model (log relative WIS $-0.54$) but
...
The absolute skill of each model carries wide uncertainty. A block bootstrap over the 104
state-season units gives a 95 percent interval on the conformal ensemble's normalized WIS of
...
units and a paired analysis removes the shared, season-driven variance (Fig~\ref{fig:significance}B).
...
worst, which mirrors its stability across seasons. Decomposing WIS into its components explains the
...
over-prediction), whereas every other season is dominated by dispersion. The record epidemic was not
...
switching the point forecast to a clamped predictive mean; a principled zero-inflated treatment is a
...
least biased total-season case count (8 percent under-prediction at the median unit, against 20
...
conformal methods for distribution shift \citep{gibbs2021,zaffran2022}, and it matches the online-recalibration
