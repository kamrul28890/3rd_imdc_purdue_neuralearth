...
XGBoost re-verification has not been adopted into the deployed ensemble pending further validation
(checking the swap at the ensemble level under this project's own fold-1 tuning discipline, the
same check used to validate the original LightGBM-to-XGBoost swap).}). A feature-importance
...
re-verification on the current XGBoost-based pipeline found the opposite (see footnote above): the
holdout's preferred configuration also won the true backtest there. Together these results locate
much of the predictable signal in this problem, caution against complexity that is not validated on
the true forecasting task, and show that even a validation discipline's own conclusions can be
implementation-specific rather than a fixed property of the problem.

\begin{table}[t]
\centering\small
\caption{\textbf{Interventions that did not help, plus one that did on re-verification (negative
results and one contradiction).} Each was evaluated on the same backtest as the main comparison.
$^{\dagger}$Originally tested on the LightGBM configuration before it was superseded by XGBoost and
before a later data refresh; re-verified on the current XGBoost-based pipeline and current data
(see footnote in text). The hyperparameter-tuning re-verification reversed the original finding.}
\label{tab:ablation}
\begin{tabular}{@{}lll@{}}
\toprule
...
Hyperparameter tuning$^{\dagger}$ & internal holdout $-2.5\%$, & generalized this time; not \\
 (XGBoost) & backtest \emph{also} better ($-3.2\%$) & yet adopted (see text) \\
\bottomrule
\end{tabular}
\end{table}
