...
(S4), a complete inventory of every model evaluated including those not retained (S5), the
EPIFORGE 2020 reporting checklist (S6), exploratory data analysis (S7), the reproducibility
statement (S8), per-state model performance (S9), calibration by percentile of the predictive
distribution (S10), and a glossary of the technical terms used in the main text (S11).
...
about 5 percent but worsened the true backtest, and was not adopted (see the negative results in the
main text).
...
\section*{S5 Table. Complete inventory of models evaluated}
Table~\ref{tab:inventory} lists every model we built and scored for the dengue state-level target,
including those that are not competitive and those we did not retain. All figures come from the
identical four-season, 26-state backtest and the same scoring code. Models marked "not retained"
were removed from the codebase after evaluation; their scores are reported here from the runs that
produced that decision, so the record is complete even though the code is not.

\begin{table}[p]
\centering\footnotesize
\caption{\textbf{S5 Table. Every model evaluated on the dengue state-level backtest.} nWIS is
normalized WIS over all four seasons; nWIS-ord excludes the 2024 outlier. Lower is better
throughout. The first block is the deployed pipeline, reported in the main text; the second is the
extended set of Section 2.8; the third was evaluated and not retained.}
\label{tab:inventory}
\begin{tabular}{@{}lrrrp{0.42\textwidth}@{}}
\toprule
\textbf{Model} & \textbf{WIS} & \textbf{nWIS} & \textbf{nWIS-ord} & \textbf{Role and outcome} \\
\midrule
\multicolumn{5}{@{}l}{\emph{Deployed pipeline}} \\
Ensemble (conformal) & 1162 & 0.561 & 0.375 & Submitted model. Best mean WIS; never worst in any season \\
XGBoost & 1190 & 0.575 & 0.434 & Ensemble member; adopted over LightGBM \\
Ensemble (Vincentization) & 1234 & 0.595 & 0.371 & Ensemble before recalibration \\
Ensemble (inverse-WIS) & 1238 & 0.598 & 0.358 & Weighted alternative; no better than the median \\
Mechanistic (trajectory) & 1238 & 0.598 & 0.450 & Reported standalone; excluded from the ensemble \\
Climatological & 1264 & 0.610 & 0.407 & Ensemble member; covariate-free baseline \\
LightGBM & 1301 & 0.628 & 0.549 & Superseded by XGBoost for dengue; retained for chikungunya \\
Seasonal-naive & 1379 & 0.666 & 0.468 & Reference baseline for relative WIS \\
GRU (negative-binomial) & 1394 & 0.673 & 0.385 & Ensemble member; best in ordinary seasons, worst on the outlier \\
Naive & 1557 & 0.752 & 0.713 & Lower reference baseline \\
\addlinespace
\multicolumn{5}{@{}l}{\emph{Extended set (Section 2.8)}} \\
Surge template & 1168 & 0.564 & 0.447 & Best single model by mean WIS; fails the tuning-fold check (0.444) \\
Hierarchical climatological & 1232 & 0.595 & 0.401 & Macroregion pooling; beats its unpooled parent, hurts the ensemble \\
Mechanistic (sub-sampled) & 1269 & 0.613 & 0.452 & Variant of the trajectory model; loses to it on all three metrics \\
GRU (pinball-trained) & 1274 & 0.615 & 0.366 & Better standalone than the deployed GRU, worse as an ensemble member \\
Climate-conditioned surge & 1406 & 0.679 & 0.677 & Surge template conditioned on temperature anomaly; a clean regression \\
SARIMAX (first pass) & 1419 & 0.685 & 0.835 & Classical family; no exogenous inputs, Gaussian intervals back-transformed \\
SARIMAX (per-fold order) & 1528 & 0.737 & 0.776 & Order search fixes most of the frozen-order gap on mean WIS \\
SARIMAX (frozen order) & 1752 & 0.846 & 1.078 & Order chosen on one fold does not transfer across regimes \\
Prophet (rebuilt) & 1520 & 0.734 & 0.648 & Flat growth, real regressors, residual bootstrap; 30\% better than v1 \\
Prophet (first pass) & 2177 & 1.051 & 1.916 & Default trend and Gaussian intervals unsuited to this horizon \\
\addlinespace
\multicolumn{5}{@{}l}{\emph{Evaluated, not retained}} \\
Random forest (quantile) & 1303 & 0.629 & 0.491 & Bagging lost to boosting on every metric; no diagnostic value kept \\
Holt-Winters (no trend) & 1525 & --- & 1.19 & Interval width grows without bound at long horizons \\
Chronos (zero-shot) & 1467 & --- & --- & Pretrained foundation model; panel too small and autocorrelated \\
Holt-Winters (damped) & 1969 & --- & 1.94 & As above, worse; both beaten by the naive baseline \\
\bottomrule
\end{tabular}
\end{table}

\section*{S6 Text. EPIFORGE 2020 reporting checklist}
...
described in S8 Text serves the same purpose, recording the exact data checksums and code revision
...
Data and code availability; S8 Text \\
...
S8 Text (provenance manifest; see the note above) \\
...
\section*{S7 Figs. Exploratory data analysis}
...
\section*{S8 Text. Reproducibility and provenance}
...
\section*{S9 Text. Per-state model performance heatmap}
...
mean and standard deviation. Blue cells are better than the baseline and red cells are worse. Two
...
relative WIS by model and state, averaged over the four seasons. Blue is better than baseline, red
is worse, and the diverging scale is centered on baseline parity. A red-blue scale is used rather
than red-green so that the comparison remains readable under red-green color vision deficiency
and in grayscale. The underlying values are released as
\texttt{log\_relative\_wis\_by\_state.csv} for readers who need the numbers rather than the
colors.}
\end{figure}

\section*{S10 Text. Calibration by percentile of the predictive distribution}
...
unequal widths (25 percentage points at the center, 2.5 at the tails), so raw counts would make
...
intervals that are uniformly too narrow rather than merely mis-centered. Second, every model,
...
too wide, and a tilt indicates bias. The tallest bar in each panel is labeled, since the shared
vertical scale is set by the deep network's outlier. Computed on all four seasons and 26 states.}
\end{figure}

\section*{S11 Text. Glossary}
